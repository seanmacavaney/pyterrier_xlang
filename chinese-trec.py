...
dataset = pt.get_dataset('irds:trec-mandarin/trec6')

topics = dataset.get_topics('title_zh', tokenise_query=False)
...
for stem in [True]:
  ...
  idx = PisaIndex(f'/home/sean/data/indices/trec-mandarin.stem-{stem}.pisa', stemmer='none', pretokenised=True, text_field=['text'])
  if not idx.built():
